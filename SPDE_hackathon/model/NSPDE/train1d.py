...
def hyperparameter_search(config):
    ...
    hyperparams = list(itertools.product(config.hidden_channels, config.n_iter, config.modes1, config.modes2))

    loss = LpLoss(size_average=False)

    fieldnames = ['hidden_channels', 'n_iter', 'modes1', 'modes2', 'nb_params', 'loss_train', 'loss_val', 'loss_test']
    ...
    for (_hidden_channels, _n_iter, _modes1, _modes2) in hyperparams:

        print('\n width:{}, L:{}, modes1:{}, modes2:{}'.format(_hidden_channels, _n_iter, _modes1, _modes2))

        model = NeuralSPDE(dim=1, in_channels=1, noise_channels=1, hidden_channels=_hidden_channels,
                           n_iter=_n_iter, modes1=_modes1, modes2=_modes2).cuda()
        ...
        with open(log_file, 'a', encoding='UTF8', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([_hidden_channels, _n_iter, _modes1, _modes2, nb_params, loss_train, loss_val, loss_test])
